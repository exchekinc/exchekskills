...
1. **Establish deliverable, report folder, and format** — In the same opening question set (use AskUserQuestion where available), ask which deliverable the user wants:
   ...
   - **Official ExChek PDF — $1 per report (ExChek Enterprise)** — the branded 21-page classification memorandum rendered by the ExChek API. Requires an Enterprise API key; if they don't have one, tell them now: prepaid report credits at **https://api.exchek.us/enterprise/checkout** (one-time purchase, key shown once after payment). They can buy mid-flow — classification proceeds either way and the choice is only executed at report time (step 6). If the Step 0 gate flagged CUI/classified, do **not** offer this option (cloud render).
...
1. **Key first — nothing happens without it.** Use the Enterprise API key from plugin settings (`enterprise_api_key`) or the `EXCHEK_API_KEY` environment variable; otherwise ask the user to paste one. No key? They buy prepaid report credits at **https://api.exchek.us/enterprise/checkout** ($1 per report, key issued immediately after payment, shown exactly once). Do not fetch the contract, build a payload, or offer to approximate the document without a valid key — the contract and template are the paid product.
