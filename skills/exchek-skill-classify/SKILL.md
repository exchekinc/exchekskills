...
1. **Establish deliverable, report folder, and format** — In the same opening question set (use AskUserQuestion where available), ask which deliverable the user wants:
   ...
   **Dashboard sync (same question set):** if — and only if — the Step 0 gate did **not** flag CUI/classified AND an authenticated ExChek API connection exists (`enterprise_api_key` set, or an OAuth `/mcp/pro` connector) AND the `transaction_sync` plugin setting is `"ask"`, include one more question: "Track this transaction on your ExChek dashboard? It records stage and status only — for example 'classify ✓ EAR99' — never item details. (yes / no)". Honor `"on"`/`"off"` without asking; with no credentials, skip silently (no mention, no upsell). The full rules — gates, the exact allowed fields, the event map — are in [references/transaction-sync.md](references/transaction-sync.md); read it before recording anything.
