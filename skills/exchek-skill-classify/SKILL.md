...
3. **Draft preview (free, mandatory)** — Render with `draft: true` (REST: `?draft=1`): a watermarked preview that consumes **no credit** and is never stored. Deliver it (file save, or the one-hour download link from the response on claude.ai — the web chat cannot accept the PDF bytes) and review with the user: document-control block (company, CAGE attribution, preparer), item line, final ECCN, exhibits populated, parties, and any render warnings. Apply corrections and re-render drafts freely until the user is satisfied. **Never render a final the user hasn't reviewed as a draft.**
4. **Finalize (1 credit)** — Ask exactly: "Finalize the memorandum? This renders the clean official PDF and uses 1 credit ($1)." Only on an explicit yes, call the tool again without `draft`. Save as `ExChek-Memorandum-YYYY-MM-DD-ShortItemName.pdf` (or hand over the download link on claude.ai; the final also lands in the dashboard vault when document storage is on).
