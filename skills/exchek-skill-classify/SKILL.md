...
5. **Record (when sync is on)** — After the final render, record `event_type: "note"`, `status: "complete"`, `ref` = the memo's DOC number on the same transaction id ([references/transaction-sync.md](references/transaction-sync.md)).
