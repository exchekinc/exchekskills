...
- **USML currency — verify against the live list.** The USML is revised periodically; a category that controlled an item in the past may have changed. A temporary Category VIII modification was terminated in early 2025, and DDTC's 2026 agenda includes revisions to **Categories IV and XV (space)**, **Category XI (semiconductor/circuit-board controls)**, and **Category IX (redefining "defense services")**. Confirm the current Part 121 text before relying on a category determination.
- **AUKUS pathway (once jurisdiction is ITAR).** Jurisdiction does not change for AUKUS, but if an item is ITAR and the activity is to/within **Australia, the United Kingdom, or the United States** among approved parties, the **§ 126.7 exemption** (final rule effective 2025-12-30) may authorize the export/reexport/retransfer without a separate license — subject to limitations: the parties must be registered/eligible U.S. persons, eligible government entities, or DDTC-listed **Authorized Users**; the defense article/service must **not** appear on the **Excluded Technology List** (Supplement No. 2 to Part 126); and value/SME limits (§§ 123.15, 124.11) apply. Note this as a possible next step in the memo when the destination is AUS/UK; do not treat it as a jurisdiction change.
