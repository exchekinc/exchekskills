...
- **Part 742 (Control Policy):** data-source gate → `get_ecfr_part` (`part: 742`) — CCL-based controls including §742.15 (encryption). Use for current regulatory text on encryption controls.
- **Full-text search:** data-source gate → `search_ecfr_part` (`part: 742`, `query: "encryption"`) — search within Part 742 for encryption-specific provisions.
