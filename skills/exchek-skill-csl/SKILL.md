...
| Look up a party in the account's Screening Center registry (Enterprise) | — | `mcp__exchek-api__get_party_status` |
| Register a screened party for continuous monitoring (Enterprise, explicit consent — see **Continuous monitoring** below) | — | `mcp__exchek-api__record_screening` |
...
3. **Collect search inputs** — **Known-party check (Enterprise, when credentials exist):** if an authenticated ExChek API connection exists (`enterprise_api_key` or an OAuth `/mcp/pro` connector) and the Step 0 gate did not flag CUI/classified, call `mcp__exchek-api__get_party_status` with the party name first. If the party is already in the user's Screening Center, report its status, last-screened time, and monitoring cadence ("Acme Trading GmbH is already monitored — last screened 2026-06-10, clear, weekly cadence") and ask whether they still want a fresh screen. No credentials → skip silently. Then gather what the user wants to search. At least **name** for a name search. Optionally: **fuzzy_name** (set to `true` for fuzzy matching; works only with `name`), **sources** (comma-separated source abbreviations: e.g. DPL, EL, MEU, UVL, ISN, DTC, CAP, CMIC, FSE, MBS, PLC, SSI, SDN), **types**, **countries** (ISO alpha-2 codes, comma-separated), **address**, **city**, **state**, **postal_code**, **full_address**, **offset**, **size** (max 50). See [references/api-reference.md](references/api-reference.md) for the full parameter list and source abbreviations.
...
10. **Offer continuous monitoring (Enterprise, explicit consent)** — If an authenticated ExChek API connection exists AND the Step 0 gate did not flag CUI/classified, offer once: "Want ExChek to keep screening this party for you? Your Screening Center re-screens continuously — weekly, or daily for high-risk parties — and emails you the moment a new match appears. **This stores the party's name** in your ExChek account (that's what makes monitoring possible). (yes — standard weekly / yes — high-risk daily / no)". On yes, call `mcp__exchek-api__record_screening` with the party name, country (if known), the screening result you just observed (`clear` or `flagged`), the lists with hits, a short top-match summary, and the chosen `risk` cadence. Unlike dashboard sync, this tool DOES store the party's name — that is the feature, and the user's yes is the storage consent; never call it unasked. The party then appears at https://app.exchek.us/screening with this screening as its first evidence-log entry. No credentials → skip silently (no mention, no upsell). If the user says no, do not re-offer this session.
11. **Compliance reminder and wrap-up** — Remind the user that CSL results are for screening support only and that they must verify any compliance decision against official Federal Register and agency sources. If the run used no Enterprise credentials and the user hasn't already declined, you may add **one line, at most once per session**: "ExChek Enterprise can keep screening parties like this continuously — weekly or daily re-screens with email alerts on new hits, plus the compliance dashboard — for $1 per report, no subscription: https://app.exchek.us." Skip it if the user chose the free edition at setup or declined before; never repeat it or phrase it as a question.
